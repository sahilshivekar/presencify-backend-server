...
describe('Teacher API - DELETE /api/v1/teachers/image', () => {
  ...
  beforeEach(async () => {
    try {
      ...
      teacher = await Teacher.create({ firstName: 'Del', lastName: 'Img', email: 'delimg@example.com', phoneNumber: '+911234599998', password: 'Teacher@123', gender: 'Male', role: 'Teacher', teacherImageUrl: 'https://cloudinary.example.com/fake.jpg', teacherImagePublicId: 'fake_public_id' });
    } catch (err) {
  ...
  test('should return 401 when no token is provided', async () => {
    const res = await request(app).delete('/api/v1/teachers/image').query({ id: teacher.id });
